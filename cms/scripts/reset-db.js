#!/usr/bin/env node
/**
 * reset-db.js — 清空 leads / campaigns / tasks，寫入 mock data 供開發測試。
 *
 * 用法：
 *   cd cms/worker && node ../scripts/reset-db.js
 *   # 或指定 URI：
 *   MONGODB_URI=mongodb://localhost:27017/lead_scraper node cms/scripts/reset-db.js
 *
 * ⚠️ 只適合開發環境，唔好喺 production 跑！
 */
const { MongoClient } = require(require('path').resolve(__dirname, '../worker/node_modules/mongodb'));
const { randomBytes } = require('crypto');

const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/lead_scraper';

function nowIso() { return new Date().toISOString().replace('T', ' ').slice(0, 19); }
function genTaskId() { return `TASK-${randomBytes(4).toString('hex')}`; }
function genCampaignId() { return `CAMP-${randomBytes(4).toString('hex')}`; }
function genLeadId() { return randomBytes(8).toString('hex'); }
...
async function main() {
  ...
  const insertedIds = Object.values(leadResult.insertedIds).map(id => id.toString());
  console.log(`✅ 寫入 ${insertedIds.length} 筆 mock leads`);

  // ── 寫入 mock campaign（包含頭 3 個 lead）──
  const campId = genCampaignId();
