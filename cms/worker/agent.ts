...
function looksLikeQuotaError(...texts: (string | undefined)[]): string | null {
  for (const text of texts) {
    if (!text) continue;
    const lower = text.toLowerCase();
    for (const p of QUOTA_PATTERNS) {
      if (lower.includes(p.toLowerCase())) return text.slice(0, 500);
    }
  }
  return null;
}
...
const QUOTA_ALERT_COOLDOWN_MS = 15 * 60_000;

/**
 * 通知後端「額度用盡」→ 後端寫通知俾每個 admin + email。
 * 原本呢件事只會出現喺 terminal log，CMS 完全睇唔到。
 */
async function reportQuotaExceeded(detail: string): Promise<void> {
  if (Date.now() - lastQuotaAlertAt < QUOTA_ALERT_COOLDOWN_MS) return;
 ...
 */
function shortenExecError(err: unknown, max = 300): string {
  const msg = (err as any)?.message ?? String(err);
  const lines = String(msg).split('\n').map((s) => s.trim()).filter(Boolean);
  if (lines.length === 0) return String(msg).slice(0, max);
 ...
 */
function hermesArgv(args: string[]): [string, string[]] {
  if (!HERMES_HOST) return [HERMES_BIN, [...HERMES_BASE_ARGS, ...args]];
...
function callHermes(prompt: string, timeoutMs = 300_000): Promise<string> {
  ...
  return new Promise((resolve, reject) => {
    execFile(bin, argv, {
      ...
      if (quota) {
        if (stderr) console.log(`[hermes] stderr: ${stderr.slice(0, 300).replace(/\n/g, '⏎')}`);
...
/** 幾久去 API 拎一次最新並行度設定 */
const CONCURRENCY_REFRESH_MS = Number(process.env.CONCURRENCY_REFRESH_MS || 30_000);
...
/**
 * 從 users collection 讀取 per-user SMTP/IMAP 設定。
 * 有值就用 user 自己嘅設定，無就 fallback 去 .env。
 */
...
async function getUserSmtpConfig(userId: string | undefined, db: Db): Promise<SmtpConfig | null> {
  if (!userId) return null;
  try {
    const u = await db.collection('users').findOne(
      { _id: new ObjectId(userId) },
      { projection: { smtpHost: 1, smtpPort: 1, smtpUser: 1, smtpPass: 1, smtpFrom: 1, imapHost: 1, imapPort: 1 } },
    );
...
async function doReplyCheck(_p: any, db: Db) {
  ...
  // 按 user_id 分組
  const userGroups = new Map<string, typeof leads>();
        ...
        for await (const msg of client.fetch({ since }, { source: true })) {
          ...
          if (!lead && from === (smtp.user || '').toLowerCase()) {
            ...
            if (cleanSubject) {
              ...
              if (eq?.lead_id) {
                const foundLead = groupLeads.find(
...
[{"name":"Example School","website":"https://example.com","email":"info@example.com","source":"google_maps"}]`;
  ...
  if (hermesOk === 0 && hermesFailures > 0) {
    throw new Error(
      `hermes 完全叫唔到（${hermesFailures} 輪全部失敗），最後錯誤：${lastHermesError}`,
    );
  }
...
interface ScrapeResult {
  emails: string[];
  phones: string[];
  whatsapp: string;       // WhatsApp 號碼（從 wa.me / api.whatsapp.com 連結提取）
    ...
    try {
      ...
      if (!whatsapp) {
        ...
        for (const pat of waPatterns) {
          ...
          if (waMatch) { whatsapp = '+' + waMatch[1]; break; }
        }
      }
...
{"email":"","phone":"","address":""}`;
  ...
  if (scraped) {
    ...
    if (scraped.whatsapp && !lead.whatsapp) set.whatsapp = scraped.whatsapp;
